...
//Copy the process entry and init the FIFO
FIFO::FIFO(const Entry &entry){
    this->entry = entry;
...
//======================= init our FIFO algorithm ====================
void FIFO::init(){
    
    //create semaphore for disk request
    ...
    int b1 = semget(1525, 1, 0666 | IPC_CREAT);
    int b2 = semget(15253, 1, 0666 | IPC_CREAT);
    
    struct sembuf sb; sb.sem_num = 0; sb.sem_flg = 0;
    
    //frame of queues for segment#, page#
    map<int,queue<pair<int, int>>> frame;
    
    //memory index
    int memoryIndex = 0;
    
    //create pipe and forkid
    int proc[2];
    int fid, fid2;
    ...
    Loop: while(memoryIndex < entry.memoryAddress.size()){
        ...
        //extract the mem address, seg#, page#
        ...
        if(frame.find(pid) != frame.end()){
            ...
            bool inFrame = false;
            queue<pair<int, int>> copyFrame = frame[pid];
            ...
            */
            if(frame[pid].size() < entry.numberOfPageFrames){
                ...
                if(fid == 0){
                    ...
                    frame[pid].push(address);
                    
                    //increase page fault to the
            ...
            /*
             ...
             first check to see if we haven't reached the min threshold
             */
...
void FIFO::deactivateProcess(int pid,map<int,queue<pair<int, int>>> *frame){
...
void FIFO::activateProcess(int pid, map<int,queue<pair<int, int>>> *frame){
